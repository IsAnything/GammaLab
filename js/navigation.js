...
window.addExposureControls = function(renderers, generateBtn) {
    ...
    controlsContainer.appendChild(right);
        ...
        console.warn('Errore in addExposureControls:', e);
    }
};
...
function setupSourceSimulator(sourceType, options = {}) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (clearBtn) {
            ...
            // --- UI: Exposure slider and Sub-pixel toggle (live control for renderers) ---
            try {
                if (typeof window.addExposureControls === 'function') {
                    window.addExposureControls(renderers, generateBtn);
                } else {
                    // Fallback: inline creation if helper not present
                    (function inlineAdd() {
                        const controlsContainer = document.createElement('div');
                        controlsContainer.style.display = 'flex';
                        controlsContainer.style.alignItems = 'center';
                ...
                console.warn('Errore creando UI exposure/subpixel:', e);
            }
